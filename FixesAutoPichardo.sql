...
Go
ALTER TABLE FormaCompraVehiculo
Drop Column id_factura_servicio

Go
ALTER TABLE FormaCompraVehiculo
ADD id_factura_servicio int not null default (0)
...
begin
	if @propietario = ''
		begin
			...
			select  CP.id_vehiculo as IDVEHICULO, CP.fecha as  'FECHA' , TP.transaccion as TRANSACCION , '0' as '#FACTURA' ,S.suplidor as 'PAGAR A',
			CONVERT(varchar(200),
			...
			and CP.id_transaccion = 2 and CP.nueva = 1
			...
			where (ISNULL(CP.balance_usd,0) > 0 or ISNULL(CP.balance_rd,0)>0) 
			and CP.id_transaccion = 2 and CP.nueva = 0 and V.id_tipo_cuenta = 1
			and format(CP.fecha,'yyyy-MM-dd') between
			format (@desde, 'yyyy-MM-dd') and format (@hasta, 'yyyy-MM-dd')

			union -- seguros

			select  CP.id_vehiculo as IDVEHICULO, CP.fecha as  'FECHA' , TP.transaccion as TRANSACCION , '0' as '#FACTURA' ,S.nombre as 'PAGAR A',
			CONVERT(varchar(200),
			(F.fabricante + ' ' + M.modelo + ' ' + CONVERT(varchar(10), V.año) + ' ' + V.color)) as VEHICULO,
			V.vin as CHASIS,
			ISNULL(CP.balance_rd, 0) as 'PENDIENTE ($RD)', ISNULL(CP.balance_usd, 0) as 'PENDIENTE ($USD)',
			...
			ISNULL(DATEDIFF(DAY, format (CP.fecha, 'yyyy-MM-dd'),
				format (GETDATE(), 'yyyy-MM-dd')), 0) as 'DIAS VIGENTE', CP.id as IDCUENTAPAGAR, CP.nueva as NUEVA
			from  CuentasPagar CP join Vehiculos V  on CP.id_vehiculo = V.id
			join SeguroVehiculo SV on SV.id_vehiculo = CP.id_vehiculo join Fabricantes F on F.id = V.fabricante_id 
			join Seguros S on S.id = SV.id_seguro
			join Modelos M on M.id = V.modelo_id  join TipoTransaccion TP on TP.id = CP.id_transaccion
			
			where (ISNULL(CP.balance_usd,0) > 0 or ISNULL(CP.balance_rd,0)>0) 
			and CP.id_transaccion = 4 and CP.nueva = 1
			and format(CP.fecha,'yyyy-MM-dd') between
			format (@desde, 'yyyy-MM-dd') and format (@hasta, 'yyyy-MM-dd')
			...
			where (ISNULL(CP.balance_usd,0) > 0 or ISNULL(CP.balance_rd,0)>0) 
			and CP.id_transaccion = 4 and CP.nueva = 0 and V.id_tipo_cuenta = 1
			and format(CP.fecha,'yyyy-MM-dd') between
			format (@desde, 'yyyy-MM-dd') and format (@hasta, 'yyyy-MM-dd')

			union

			select   CP.id_vehiculo as IDVEHICULO, CP.fecha as  'FECHA' ,TP.transaccion as TRANSACCION , FS.facturaSuplidor as '#FACTURA' , TR.taller as 'PAGAR A',
			CONVERT(varchar(200),
			(F.fabricante + ' ' + M.modelo + ' ' + CONVERT(varchar(10), V.año) + ' ' + V.color)) as VEHICULO,
			V.vin as CHASIS,
			ISNULL(CP.balance_rd, 0) as 'PENDIENTE ($RD)', ISNULL(CP.balance_usd, 0) as 'PENDIENTE ($USD)',
			ISNULL(DATEDIFF(DAY, format (CP.fecha, 'yyyy-MM-dd'),
				format (GETDATE(), 'yyyy-MM-dd')), 0) as 'DIAS VIGENTE', CP.id as IDCUENTAPAGAR, CP.nueva as NUEVA
			from Vehiculos V  join CuentasPagar CP on CP.id_vehiculo = V.id
			join Fabricantes F on F.id = V.fabricante_id 
			join Modelos M on M.id = V.modelo_id  join TipoTransaccion TP on TP.id = CP.id_transaccion
		...
		begin
			...
			and format(CP.fecha,'yyyy-MM-dd') between
			format (@desde, 'yyyy-MM-dd') and format (@hasta, 'yyyy-MM-dd')
			and TR.taller LIKE '%' + @propietario + '%'
...
begin
	...
	((select ISNULL(SUM(PCC.monto_rd),0) from PagosCuentasCobrar PCC join CuentasCobrar CC on CC.id = PCC.id_cuentaCobrar and CC.id_vehiculo = V.id)
	 + (select sum(monto_rd) from FormaVentaVehiculo FV where FV.id_transaccion = 1 and Fv.id_tipo_pago !=3 and FV.id_vehiculo = V.id) ) as PAGADORD ,
	V.precioVentaUSD as PRECIOUSD,
	((select ISNULL(SUM(PCC.monto_usd),0) from PagosCuentasCobrar PCC join CuentasCobrar CC on CC.id = PCC.id_cuentaCobrar and CC.id_vehiculo = V.id)
	 + (select sum(monto_usd) from FormaVentaVehiculo FV where FV.id_transaccion = 1 and Fv.id_tipo_pago !=3 and FV.id_vehiculo = V.id) ) as PAGADOUSD ,
...
begin
	...
	C.cliente as CLIENTE,
	ISNULL((select monto_rd from FormaVentaVehiculo where id_transaccion = 1 and id_tipo_pago = 1 and V.id = id_vehiculo),0) as EFECTIVO, 
	ISNULL((select monto_rd from FormaVentaVehiculo where id_transaccion = 1 and id_tipo_pago = 2 and V.id = id_vehiculo),0) as VRECIBIDO,
	ISNULL((select monto_rd from FormaVentaVehiculo where id_transaccion = 1 and id_tipo_pago = 3 and V.id = id_vehiculo),0) as CREDITO, 
	ISNULL((select monto_rd from FormaVentaVehiculo where id_transaccion = 1 and id_tipo_pago = 4 and V.id = id_vehiculo),0) as OTROS
...
begin
	...
	C.cliente as CLIENTE,
	ISNULL((select monto_usd from FormaVentaVehiculo where id_transaccion = 1 and id_tipo_pago = 1 and V.id = id_vehiculo),0) as EFECTIVO, 
	ISNULL((select monto_usd from FormaVentaVehiculo where id_transaccion = 1 and id_tipo_pago = 2 and V.id = id_vehiculo),0) as VRECIBIDO,
	ISNULL((select monto_usd from FormaVentaVehiculo where id_transaccion = 1 and id_tipo_pago = 3 and V.id = id_vehiculo),0) as CREDITO, 
	ISNULL((select monto_usd from FormaVentaVehiculo where id_transaccion = 1 and id_tipo_pago = 4 and V.id = id_vehiculo),0) as OTROS
	from Vehiculos V join Fabricantes F on V.fabricante_id = F.id join Modelos M on V.modelo_id = M.id
	join Propietarios P on V.id_propietario = P.id 
	join Clientes C on V.id_cliente = C.id 
	and format(V.fecha_vendido,'yyyy-MM-dd') between
...
begin
	if @cliente = ''
		begin
			...
			PCC.monto_usd as 'MONTO $(USD)', CC.id as '# CUENTA COBRAR',  CC.nueva as NUEVA
			from 
...
update CuentasPagar set id_transaccion = 8 where id_transaccion = 5 or id_transaccion = 6 or id_transaccion = 7


GO
	alter procedure [dbo].[insertarFormaGastos]
	...
	set @mensaje = 0;
	begin
		insert into FormaCompraVehiculo (id_vehiculo, id_transaccion, id_tipo_pago, id_factura, monto_rd, monto_usd, nota, fecha, id_factura_servicio)
		VALUES (@idVehiculo, @idTransaccion, @idTipoPago, 0, @montoRD, @montoUSD, @nota, @fecha, @idFactura)
		...
		if @idTipoPago = 3 
			begin
				 insert into CuentasPagar (id_factura, id_vendedor, id_transaccion, monto_rd, monto_usd,
				  ...
				  VALUES(@idFactura, @idSuplidor, 8, @montoRD, @montoUSD, @montoRD, @montoUSD,
				  @fecha, @idVehiculo)
...
@idVehiculo int, @idTransaccion int, @idCP int
as
...
begin
	if @idTransaccion = 8
		begin
			declare @idFacturaServicio int= (select id_factura from CuentasPagar where id = @idCP)
			select Format(FC.fecha,'dd/MM/yyyy') as 'FECHA DE PAGO', TP.formaPago as 'TIPO PAGO' ,
			...
			where FC.id_vehiculo = @idVehiculo and id_factura_servicio = @idFacturaServicio
		end
	else
		begin
...
update FormaCompraVehiculo set id_factura_servicio = id_factura, id_factura = 0
...
begin
	...
	(V.precioVentaRD + Isnull(PS.precioRD, 0) + PT.precioRD) as PRECIORD, (V.precioVentaUSD + isnull(PS.precioUSD, 0) + PT.precioUSD) as PRECIOUSD,
...
GO
ALTER procedure [dbo].[obtener_detalle_cuenta_por_cobrar]
@idVehiculo int
as
...
begin
	...
	from Vehiculos V join Fabricantes F on V.fabricante_id = F.id join Modelos M on V.modelo_id = M.id
	join Clientes C on V.id_cliente = C.id left join PreciosSeguroVehiculo PS on PS.id_vehiculo = V.id
...
ALTER procedure [dbo].[obtener_detalle_cuenta_cobrar_antigua]
@idCC int
as
...
begin
	...
	where D.id_tipo_cuenta = 2 and CC.nueva = 0 and CC.id = @idCC and D.id_tipo_cuenta = 2
end

go

ALTER procedure [dbo].[registrar_pago_cuenta_cobrar]
...
begin
	...
	if @idTipoPago = 1
			begin
				 ...
				 values (@idVehiculo, 2, @montoRD, @montoUSD, @fecha, 'CxC RECIBO PAGO', (select MAX(id) from PagosCuentasCobrar))
			end
	set @mensaje = 1;
end
...
begin
	...
	if @idTipoPago = 1
		begin
			insert into DetallesEfectivoGeneral(idVehiculo, tipoCuenta, montoRD, montoUSD, fecha, documento, numeroDocumento)
			values (@idVehiculo, 1, @montoRD, @montoUSD, @fecha, 'CxP RECIBO PAGO', (select MAX(id) from PagosCuentasPagar))
		end
...
begin 
	...
	join Clientes C on C.id = V.id_cliente
	where DEG.tipoCuenta = 2 
	and format(DEG.fecha,'yyyy-MM-dd') between
	format (@desde, 'yyyy-MM-dd') and format (@hasta, 'yyyy-MM-dd')

	union

	select DEG.fecha as FECHACOBRO
	, V.chasis as CHASIS
	, DEG.montoRD as MONTORD,
	DEG.montoUSD as MONTOUSD,
	 C.cliente as CLIENTE, 
	(DEG.documento + ' #' +  Convert (varchar(50),FORMAT(DEG.numeroDocumento, '000000'))) as DOCUMENTO
	from DetallesEfectivoGeneral DEG  join PagosCuentasCobrar PC on PC.id = DEG.numeroDocumento
	join Detalle_CC_CP_Antiguas V on V.id_cuenta = PC.id_cuentaCobrar
	join CuentasCobrar CC on CC.id = PC.id_cuentaCobrar
	join Clientes C on C.id = CC.id_cliente
	where DEG.tipoCuenta = 2  and v.id_tipo_cuenta = 2 and DEG.idVehiculo = 0
		and format(DEG.fecha,'yyyy-MM-dd') between
	format (@desde, 'yyyy-MM-dd') and format (@hasta, 'yyyy-MM-dd')
end
...
go
if not exists (select * from activation)
			begin
				insert into activation (activated)
				Values (0)
			end
...
ALTER procedure [dbo].[activar_software]
...
GO
ALTER procedure [dbo].[llenar_tablas]
as
